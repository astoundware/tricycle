...
import {VideoAspectRatioPresetTable} from '@components';
import '@config/i18n';
import {VideoAspectRatioPreset} from '@models';
import {
  createListRenderItemInfo,
  createNumber,
  createString,
  createVideoAspectRatioPreset,
} from '@test-utils/fixtures';

let element: ListRenderItemInfo<VideoAspectRatioPreset>;

beforeEach(() => {
  element = createListRenderItemInfo(createVideoAspectRatioPreset);
});

it('renders the row name correctly', () => {
  const component = renderer.create(<VideoAspectRatioPresetTable />);
...
it("calls onNameChange when the row's name value is changed", () => {
  ...
  const component = renderer.create(
    <VideoAspectRatioPresetTable onNameChange={onNameChange} />,
  );
  const list = component.root.findByType(FlatList);
  const row = list.props.renderItem(element);
...
it("calls onWidthChange when the row's width value is changed", () => {
  ...
  const component = renderer.create(
    <VideoAspectRatioPresetTable onWidthChange={onWidthChange} />,
  );
  const list = component.root.findByType(FlatList);
  const row = list.props.renderItem(element);
...
it("calls onHeightChange when the row's height value is changed", () => {
  ...
  const component = renderer.create(
    <VideoAspectRatioPresetTable onHeightChange={onHeightChange} />,
  );
  const list = component.root.findByType(FlatList);
  const row = list.props.renderItem(element);
